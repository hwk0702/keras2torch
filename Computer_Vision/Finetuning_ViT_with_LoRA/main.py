...
def fit(
    args, exp_name, model, epochs, trainloader, testloader, criterion, optimizer, scheduler, 
    savedir, log_interval, accumulation_steps=1, device='cpu'
):
    savedir = os.path.join(savedir,exp_name)
    os.makedirs(savedir, exist_ok=True)
    wandb.init(name=exp_name, project='ViT LoRA', config=args)
    ...
    for epoch in range(epochs):
        ...
        if best_acc < eval_metrics['acc']:
            ...
            if args.apply_lora:
                weights = {'lora':lora.lora_state_dict(model)}
            else:
...
def run(args):
    ...
    # Build Model
    model = create_model('vit_base_patch16_224', pretrained=True, img_size=224, num_classes=100, apply_lora=args.apply_lora, lora_r=args.lora_r, lora_alpha=args.lora_alpha)
    if args.apply_lora:
        lora.mark_only_lora_as_trainable(model)
    model.to(device)

    total_param = np.sum([p.numel() for p in model.parameters()])
    train_param = 0
    for p in model.parameters():
        if p.requires_grad:
            train_param += p.numel()

    _logger.info('# of model params: {}'.format(total_param))
    _logger.info('# of learnable params: {}'.format(train_param))
...
if __name__=='__main__':
    ...
    parser.add_argument('--apply_lora',default=False,action='store_true',help='apply LoRA')
    parser.add_argument('--lora_r',type=int,default=8,help='rank of LoRA')
    parser.add_argument('--lora_alpha',type=int,default=8,help='scaling factor of LoRA')
